...
WORD wincpuidext() {
	...
	*((int*)vendor_id) = cpuInfo[1];
	*((int*)(vendor_id + 4)) = cpuInfo[3];
	*((int*)(vendor_id + 8)) = cpuInfo[2];
	nMaxCode = cpuInfo[0];
	...
	if ( wincpuidsupport() )
	{
		...
		{      
			...
			mov     nMaxCode, eax // save max code
		...
		_asm
		{
			...
			mov     eax, nMaxCode	// restore max code
