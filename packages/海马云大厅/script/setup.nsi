...
Var InstalledYYGameVersion ; 已经安装的YY游戏大厅版本
Var AppdataYYGameVersion ; 在Ver\0读到的YY游戏大厅版本
Var RegeditYYGameVersion ; 在注册表读到的YY游戏大厅版本
Var HighestVersion       ; 上面几个版本中最高的那个
Var HighestVersionPath   ; 最新的版本的路径
...
Function GUIInstall
	...
	Call CheckAppdataYYGameVer
	
	Call CheckRegeditYYGameVer
	
	Call GetHighestVersion
	
  ${VersionCompare} $HighestVersion ${PRODUCT_VERSION} $R9
  
  ${If} $R9 = "2"
    ${NSDM_DEBUG} "HMY: 覆盖安装，进入消息循环$\n"
		${nsdm.Run}
  ${Else}
    ${NSDM_DEBUG} "HMY: 已经安装了$HighestVersionPath版本大厅$\n"
    Exec '"$HighestVersionPath" ${PRODUCT_DESKTOP_ARG}'
  	!insertmacro PostPasEvent ${PAS_RUN_CLIENT}
    StrCpy $InstallState "done"
		Call OnExitDUISetup
  ${EndIf}
...
Function CheckInstalled
	ReadRegStr $1 ${PRODUCT_ROOT_KEY} "${PRODUCT_SUB_KEY}" "version"
	StrCpy $InstalledYYGameVersion 0
	${If} "$1" == ""
		; 兼容旧版本
		ReadRegStr $1 "HKLM" "${PRODUCT_SUB_KEY}" "version"
		${If} "$1" == ""
	...
	StrCpy $InstalledYYGameVersion "$1"
	
FunctionEnd

Function CheckAppdataYYGameVer

  StrCpy $AppdataYYGameVersion 0
	StrCpy $9 ""
  IfFileExists "$APPDATA\duowan\yygame5\ver\launch.ini" 0 +2
    ReadINIStr $9 "$APPDATA\duowan\yygame5\ver\launch.ini" "" "newver"
    
  ${If} "$9" != ""
    IfFileExists "$APPDATA\duowan\yygame5\ver\$9\${PRODUCT_MAIN_EXE}" 0 +2
       StrCpy $AppdataYYGameVersion $9
	${EndIf}

FunctionEnd

Function CheckRegeditYYGameVer

	StrCpy $RegeditYYGameVersion 0
	ReadRegStr $9 "HKCU" "Software\duowan\YYGame5" "CurrentVerPath"
  IfFileExists "$9ygrc\version.ini" 0 +2
    ReadINIStr $RegeditYYGameVersion "$9ygrc\version.ini" "" "localver"

FunctionEnd

Function GetHighestVersion

  StrCpy $HighestVersion 0
  StrCpy $HighestVersionPath 0
  
  ${VersionCompare} $InstalledYYGameVersion $HighestVersion $R9
  ${If} $R9 = "1"
    StrCpy $HighestVersion $InstalledYYGameVersion
    ReadRegStr $HighestVersionPath ${PRODUCT_ROOT_KEY} "${PRODUCT_SUB_KEY}" "installDir"
  ${EndIf}

  ${VersionCompare} $AppdataYYGameVersion $HighestVersion $R9
  ${If} $R9 = "1"
    StrCpy $HighestVersion $AppdataYYGameVersion
    StrCpy $HighestVersionPath "$APPDATA\duowan\yygame5\ver\$AppdataYYGameVersion"
  ${EndIf}

  ${VersionCompare} $RegeditYYGameVersion $HighestVersion $R9
  ${If} $R9 = "1"
    StrCpy $HighestVersion $RegeditYYGameVersion
    ReadRegStr $HighestVersionPath "HKCU" "Software\duowan\YYGame5" "CurrentVerPath"
  ${EndIf}

  StrCpy $HighestVersionPath "$HighestVersionPath\${PRODUCT_MAIN_EXE}"
...
Function SetInstallFolder
	...
	${AndIf} "${NSDM_OUT}" != ""
		...
		${nsdm.SetControlAttr} "edit_install_folder" "$APPDATA\duowan\yygame5\ver\${PRODUCT_VERSION}" "text"
...
Function WriteLaunchIni

  FileOpen  $9 "$APPDATA\duowan\yygame5\ver\launch.ini" w
	FileWrite $9 "[]$\r$\n"
	FileWrite $9 "oldver=${PRODUCT_VERSION}$\r$\n"
	FileWrite $9 "newver=${PRODUCT_VERSION}$\r$\n"
	FileClose $9 
