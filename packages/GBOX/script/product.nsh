...
!define    PRODUCT_CHANNEL_NAME          "official"                                                         ;分端渠道号
!define    PRODUCT_CHANNEL_TAG           "23326"                                                           ;分端标识
!define    PRODUCT_CLIENT_EXE            "glauncher.exe;gbrowser.exe;gbmonitor32.exe;gbmonitor64.exe"                                      ;客户端主进程名，用户安装/卸载之前检测进程是否在运行
...
!define    PRODUCT_INSTALL_SIZE          146800640                                                          ;产品安装需要占用的磁盘字节数(140M)

Name       "${PRODUCT_NAME}"            ;安装程序名字
OutFile    "gbox.exe"             ;安装包名字
;InstallDir "$PROGRAMFILES\duowan\${PRODUCT_NAME}" ;默认安装位置
InstallDir "$APPDATA\gbox" ;默认安装位置
